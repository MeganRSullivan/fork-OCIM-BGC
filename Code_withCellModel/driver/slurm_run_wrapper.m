...
diary logs/optPCO_Tz_prescribe_C2P_N23in_NPPp2c_Tz_optGBC2024.out
%fprintf('----------------------------------- \n\n')
dbstop if error
...
fprintf('optimizing the PCO model using the previously optimized temperature-dependent model from my 2024 GBC paper for both the C2P of uptake, and the p2c conversion for satellite NPP. To see if using consistent C:P in NPP and uptake gives better objective function value. using the SetUp input fields from the Nature paper (Wang et al., 2023) using initial guess on CO and params from from hojongs previous run. no smoothing for Temp obs or DIP_obs \n\n')
