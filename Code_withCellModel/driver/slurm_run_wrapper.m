...
diary logs/optPCO_GM15_N23in_npp2_CAFE_NPPp2c_const.out
%fprintf('----------------------------------- \n\n')
dbstop if error
date_start = datetime('now');
fprintf('Start Date: %s\n',date_start)
fprintf('Reoptimizing the model from Nature paper (Wang et al., 2023), using CAFE NPP field and replacing the p2c conversion for satellite NPP with a constant =1/117. using default initial parameters and initCO. Goal: to see if the optimal c2p is still sensitive to the NPP p2c conversion when using CAFE NPP. \n\n')
