...
diary logs/optPCO_Cell_with_dop_prescribeCellO2C_npp1_NPPp2cGM15.out
%fprintf('----------------------------------- \n\n')
dbstop if error
...
fprintf('optimizing the PCO model using the cellular growth C:P model. Use the SetUp input fields from the Nature paper (Wang et al., 2023); using initial guess on CO and params from previous reoptNature_with_dop_npp1 run. smoothing both Temp obs and DIP_obs; sing CbPM NPP and GM15 p2c for satellite NPP conversion; prescribe O2C field from Cell Model GBC2024 optimization. GOAL: to  see if model can find reasonable C2P and fit to obs if we force a more realiztic O2C ratio. (prior optimization for rO2C has been finding rO2C higher than is biologically feasible) \n\n')
