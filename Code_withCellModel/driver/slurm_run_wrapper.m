...
fprintf('continuing the optimization: optimizing the PCO model using the previously optimized cellular growth C:P model from my 2024 GBC paper for both the C2P of uptake and the p2c conversion for satellite NPP; also using optimized cellular growth model for O2C. GOAL: To compute sequestration C:P as a function of residence time in my dissertation. (so chapter 3 builds on the output of chapter 2). Use the SetUp input fields from the Nature paper (Wang et al., 2023) using initial guess on CO and params from previous optPCO_Cell_prescribe_C2P run. smoothing both Temp obs and DIP_obs \n\n')
